...
class WanNetworkTrainer(NetworkTrainer):
    ...
    def do_inference(
        ...
        if self.i2v_training or self.control_training:
            image_latents_concat =  torch.concat([image_latents, reference_image_latents], dim=2)
            arg_c["y"] = image_latents_concat
            arg_null["y"] = image_latents_concat 
        ...
        with torch.no_grad():
            for i, t in enumerate(tqdm(timesteps, desc=f"Sampling timesteps for prompt {prompt_idx + 1}")):
                ...
                latent_model_input = [torch.concat([latent.to(device=device), noisy_condi[0]], dim=1)]
    ...
    def _call_dit(
        ...
        if latents.shape[1] > 16:
            ...
            noisy_input_concat = torch.concat([noisy_model_input, noisy_condi], dim=2)
            image_latents_concat =  torch.concat([image_latents, conditional_image_latents], dim=2)


            with accelerator.autocast():
                model_pred = model(noisy_input_concat, t=timesteps,  context=context, clip_fea=clip_fea, seq_len=seq_len, y=image_latents_concat)
                model_pred = model(noisy_input_concat, t=timesteps, 
                                        context=context, clip_fea=clip_fea, 
                                        seq_len=seq_len, y=image_latents_concat)
            model_pred = [pred[:,:lat_f] for pred in model_pred]
        else:
            #print("Train 2 ******")
            # call DiT
            lat_f, lat_h, lat_w = latents.shape[2:5]
