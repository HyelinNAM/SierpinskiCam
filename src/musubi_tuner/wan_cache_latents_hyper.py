...
def encode_and_save_batch(vae: WanVAE, clip: Optional[CLIPModel], i2v: bool, batch: list[ItemInfo], one_frame: bool = False):
    ...
    _, _, _, H, W = contents.shape
    Hh = (H // 32) * 32
    Ww = (W // 16) * 16

    if Hh != H or Ww != W:
        contents = Fi.interpolate(contents, size=(contents.shape[2], Hh, Ww), mode='trilinear', align_corners=False)
    h, w = Hh // 2, Ww

    rgb_con = contents[:, :, :, :h, :w] / 255.0
    point_con = contents[:, :, :, h:, :w] / 255.0

    rgb_con = rgb_con * 2.0 - 1.0
    point_con = point_con * 2.0 - 1.0 # leave what it be
    ...
    with torch.amp.autocast(device_type=vae.device.type, dtype=vae.dtype), torch.no_grad():
        ...
        rgb_lat = vae.encode(rgb_con)
        point_lat = vae.encode(point_con)

    rgb_lat = torch.stack(rgb_lat, dim=0)  # B, C, F, H, W
    point_lat = torch.stack(point_lat, dim=0)  # B, C, F, H, W

    latent = torch.concat([rgb_lat, point_lat], dim=1)  # B, C, F, H, W
    ...
    if i2v:
        ...
        images = rgb_con[:, :, 0:1, :, :]  # B, C, F, H, W, non contiguous view is fine
    ...
    if batch[0].control_content is not None:
        ...
        control_contents = Fi.interpolate(control_contents, size=(control_contents.shape[2], h, w), mode='trilinear', align_corners=False)
        with torch.amp.autocast(device_type=vae.device.type, dtype=vae.dtype), torch.no_grad():
