...
def collect_sorted_frames(folder_path, limit=49):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Create SierpinskiCam conditioning videos from an image-sequence dataset.")
    parser.add_argument("--input-base", required=True, help="Directory containing one subdirectory per scene with input frames.")
    parser.add_argument("--output-base", required=True, help="Output directory for camera-specific rgb/dense/mask/dome/dense_tx folders.")
    parser.add_argument("--camera-path", required=True, help="camera_path.json generated by scripts/generate_camera_path.py.")
    ...
    parser.add_argument("--camera-names", default=None, help="Comma-separated camera names to render. Defaults to cam01-cam10.")
    parser.add_argument("--scenes", default=None, help="Comma-separated scene names to process. Defaults to all scenes.")
    ...
    folders = [d for d in os.listdir(args.input_base) if os.path.isdir(os.path.join(args.input_base, d))]
    folders.sort()
    if args.scenes:
        requested_scenes = {name.strip() for name in args.scenes.split(",") if name.strip()}
        folders = [scene for scene in folders if scene in requested_scenes]

    skip_output_name = "dense_tx" if "dense_tx" in folder_names else folder_names[0]
    for scene in progress_bar(folders):
        video_folder = os.path.join(args.input_base, scene)
        expected_output_file = os.path.join(output_folders[camera_names[-1]][skip_output_name], f"{scene}.mp4")

        if os.path.exists(expected_output_file):
            continue

        frame_paths = collect_sorted_frames(video_folder)
